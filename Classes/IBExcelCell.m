...
@interface IBExcelCell()



@end

@implementation IBExcelCell
...
-(void)setMergeCellColumAndRowStr:(NSString *)mergeCellColumAndRowStr
{
    ...
    if(mergeCellColumAndRowStr && [mergeCellColumAndRowStr isKindOfClass:[NSString class]] &&mergeCellColumAndRowStr.length > 0)
    {
        ...
        NSString *rowStr = [IBExcelCell getNumberFromStr:mergeCellColumAndRowStr];
        
        self.mergeRow = [rowStr integerValue];
        
        NSString *column = [IBExcelCell getLetterFromStr:mergeCellColumAndRowStr];
...
-(void)refreshData
{
    ...
    NSString *rowStr = [IBExcelCell getNumberFromStr:r];
    
    self.row = [rowStr integerValue];
    
    NSString *column = [IBExcelCell getLetterFromStr:r];
