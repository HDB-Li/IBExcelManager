...
@implementation IBExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            IBExcelSheet *sheet = [[IBExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [IBExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
...
/**
 ...
 @return 工作表
 */
-(IBExcelSheet *)getSheetWithSheetName:(NSString *)sheetName
{
...
-(NSDictionary *)getSharedStringsWithPath:(NSString *)path
{
    ...
    if(xmlData && xmlData.length > 0)
    {
        ...
        xmlDic = [XMLReader dictionaryForXMLData:xmlData error:&error];
    }
    return xmlDic;
}
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[IBExcelCell getNumberFromStr:fileName] integerValue];
...
-(NSArray *)getSheedNameInfoArrayWithPath:(NSString *)path
{
    ...
    NSDictionary *sheetNameInfoDic = [XMLReader dictionaryForXMLData:xmlData error:&error];
